Sub PrintAndTransferDataNonMedicare()
    ...
    Set wsSource = ThisWorkbook.Sheets("NON MEDICARE POD")
    ...
    ' Transfer data from NON MEDICARE POD to ORDER LIST
    With wsSource
        wsTarget.Cells(NextRow, 1).Value = .Range("B6:F6").Cells(1, 1).Value ' Name
        wsTarget.Cells(NextRow, 2).Value = .Range("B7:F7").Cells(1, 1).Value ' Address
        wsTarget.Cells(NextRow, 3).Value = .Range("B8:F8").Cells(1, 1).Value ' Phone Number
        wsTarget.Cells(NextRow, 4).Value = .Range("B9:F9").Cells(1, 1).Value ' Date of Birth
        wsTarget.Cells(NextRow, 5).Value = .Range("B10:F10").Cells(1, 1).Value ' Insurance
        wsTarget.Cells(NextRow, 6).Value = Date ' Date of Service

        ' Part 1
        partID = Trim(.Range("A13:C14").Cells(1, 1).Value)
        quantity = .Range("B13:C14").Cells(1, 1).Value
        If partID <> "" Then
            ...
            wsTarget.Cells(NextRow, 9).Value = .Range("C13:C14").Cells(1, 1).Value ' Side 1
            AdjustInventory wsInventory, partID, CDbl(quantity)
        End If

        ' Part 2
        partID = Trim(.Range("A19:C20").Cells(1, 1).Value)
        quantity = .Range("B19:C20").Cells(1, 1).Value
        If partID <> "" Then
            If IsEmpty(quantity) Or quantity = "" Then
            ...
            wsTarget.Cells(NextRow, 12).Value = .Range("C19:C20").Cells(1, 1).Value ' Side 2
            AdjustInventory wsInventory, partID, CDbl(quantity)
        End If

        ' Part 3
        partID = Trim(.Range("A25:C26").Cells(1, 1).Value)
        quantity = .Range("B25:C26").Cells(1, 1).Value
        If partID <> "" Then
            If IsEmpty(quantity) Or quantity = "" Then
            ...
            wsTarget.Cells(NextRow, 15).Value = .Range("C25:C26").Cells(1, 1).Value ' Side 3
            AdjustInventory wsInventory, partID, CDbl(quantity)
        End If

        ' Part 4
        partID = Trim(.Range("A31:C32").Cells(1, 1).Value)
        quantity = .Range("B31:C32").Cells(1, 1).Value
        If partID <> "" Then
            If IsEmpty(quantity) Or quantity = "" Then
            ...
            wsTarget.Cells(NextRow, 18).Value = .Range("C31:C32").Cells(1, 1).Value ' Side 4
            AdjustInventory wsInventory, partID, CDbl(quantity)
        End If
    End With
